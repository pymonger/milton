...
/**
 * An object that performs the client-side operations needed to generate ZSync PUT data.<p/>
 * In order to update a file on a server, the client first needs to download the appropriate
 * .zsync containing the metadata of the server file. The client should then instantiate an <code>UploadMaker</code>, 
 * passing to the constructor this .zsync file as well as the local file to be uploaded.
 * On construction, the <code>UploadMaker</code> will determine the data ranges and assembly instructions 
 * that need to be sent to the server, and will automatically fill in an internal <code>Upload</code>
 * object. The client can then invoke the <code>getInputStream</code> method, which will return a stream
 * which should be used as the body of a PUT request.<p/>
 * 
 * E.g: <p/>
 * <code>
 * 
 * UploadMaker um = new UploadMaker(File clientFile, File zsFile);<br/>
 * InputStream putData = um.getInputStream();<p/>
 * 
 * </code>
 * 
 * Note: This is one of two classes that can be used to create a ZSync upload. The other
 * class, <code>UploadMakerEx</code>, performs the same functions but may perform better for certain 
 * rare cases.
 * 
 * 
 * @author Nick
...
//			System.out.println(" " + ( t3 - t2 ) + " milliseconds");
	...
	/**
	 * Initializes MetaFileReader and MakeContext objects from the .zsync file and
	 * finds and maps the matching blocks.
	 * 
	 */
	...
	/**
	 * Invokes the sequence of methods to generate the upload data and fill in the internal Upload object.
	 *
	 * @throws IOException 
	 */
	...
	/**
	 * Determines the byte ranges of new data that need to be sent to the server to update its file.<p/>
	 * 
	 * The <code>fileMap</code> argument should be an array that maps matching blocks from
	 * the server's file (the side that sent the metadata) to those in the client file, such that 
	 * <code>fileMap[seq] == off</code> means that block number <code>seq</code> in the server's file matches 
	 * the block in the local file beginning at byte <code>off</code>. An invalid offset is ignored and should
	 * be used to indicate that the local file contains no match for that block. The <code>fileMap</code> array 
	 * can be obtained from the MakeContext class.<p/>
	 * 
	 * @param fileMap An array mapping blocks in server file to their offsets in local file
	...
	/**
	 * Returns the assembly instructions needed by the server to relocate the blocks it already has. <p/>
	 * 
	 * The <code>combineRanges</code> argument determines whether contiguous matching blocks
	 * should be combined into a single range, e.g. given a blockSize of 100, whether 0-10/500, 10-20/600, 
	 * 20-30/700 should be combined into the single RelocateRange of 0-30/500. 
	 * 
	 * @param fileMap An array mapping blocks in the server file to their matches in the local file
	 ...
	 * 
	 * @param fileMap The array mapping matching blocks, obtained from MakeContext
	 * @param blockSize The number of bytes in a block
	 * @param blockIndex The index of the first block of the sequence
	 * @return A Range beginning at blockIndex that is to be relocated as a single chunk
	 */
	 ...
	 * 
	 * Note: Any temporary files used to store the data for the stream will be deleted once
